...
public class HTMLAutoComplete implements AutoCompleteProvider {
	...
	int column) {
		...
		for (String attr : HTMLLanguage.ATTRIBUTES)
			if (attr.startsWith(prefix))
				items.add(attrAsCompletion(attr, "Attribute"));

		for (String ddd : HTMLLanguage.JS)
			if (ddd.startsWith(prefix))
		...
		for (String post : HTMLLanguage.POSTEEMET)
			if (post.startsWith(prefix))
				items.add(getEmmetItemPost(post, "Emmet Spined"));
	...
	private CompletionItem getEmmetItemPost(String em, String doc) {
		...
		final CompletionItem item = new CompletionItem(em, doc);
		item.cursorOffset(item.commit.length() - 1);
		return item;
	}
